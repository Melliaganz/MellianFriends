...
function isLogged() {
  const loggedIn = Cookies.get("MellianFriends");
  if (loggedIn === "true") {
    return true;
  } else {
    return false;
...
function getIdFromCookie() {
  const mellianFriendsId = Cookies.get("mellianFriendsId");
  if (mellianFriendsId) {
    return mellianFriendsId;
  } else {
    return false;
  }
}
...
function logout(page) {
  Cookies.remove("MellianFriends");
  Cookies.remove("mellianFriendsId");
